export declare const workspaceDictionary: {
    en: {
        WKS_API_WORKSPACE_FORBIDDEN: string;
        WKS_WORKSPACE_NOT_FOUND: string;
        WKS_CONNECTION_INTERFACE_ALREADY_EXISTS_FOR_ACCOUNT: string;
        WKS_CONNECTION_INTERFACE_NOT_AVAILABLE: string;
        WKS_WORKSPACE_ALREADY_EXISTS_FOR_ACCOUNT: string;
        WKS_THERE_IS_A_SHARED_INFRA_WITH_THIS_STACK_DELETE_ERROR: string;
        WKS_THERE_IS_AN_APPLICATION_WITH_THIS_STACK_DELETE_ERROR: string;
        WKS_APPLICATION_LINK_NAME_ALREADY_EXISTS: string;
        WKS_APPLICATION_LINK_URL_ALREADY_EXISTS: string;
        WKS_SHARED_INFRA_LINK_NAME_ALREADY_EXISTS: string;
        WKS_SHARED_INFRA_LINK_URL_ALREADY_EXISTS: string;
        WKS_WORKSPACE_LINK_NAME_ALREADY_EXISTS: string;
        WKS_WORKSPACE_LINK_URL_ALREADY_EXISTS: string;
        WKS_ATTR_INVALID: string;
        WKS_ATTR_INVALID_EVALUATED_FROM_VALIDATIONS_LENGTH: string;
        WKS_BAD_REQUEST_ERROR: string;
        WKS_THERE_ARE_CLOUD_ACCOUNT_FOR_THIS_WORKSPACE_DELETE_ERROR: string;
        WKS_THERE_ARE_STACK_IN_THIS_WORKSPACE_DELETE_ERROR: string;
        WKS_THERE_ARE_CONNECTION_INTERFACE_IN_THIS_WORKSPACE_DELETE_ERROR: string;
        WKS_THERE_IS_RUNNING_DEPLOY_FOR_THIS_APPLICATION: string;
        WKS_THERE_ARE_ACTIVATED_DEPLOY_FOR_THIS_APPLICATION: string;
        WKS_THERE_IS_CONNECTION_INTERFACE_FOR_THIS_APPLICATION_DELETE_ERROR: string;
        WKS_THERE_IS_RUNNING_DEPLOY_FOR_THIS_SHARED_INFRA: string;
        WKS_THERE_ARE_ACTIVATED_DEPLOY_FOR_THIS_SHARED_INFRA: string;
        WKS_THERE_IS_CONNECTION_INTERFACE_FOR_THIS_SHARED_INFRA_DELETE_ERROR: string;
        WKS_WORKSPACE_EMBEDDED_LINK_NOT_FOUND: string;
        WKS_WORKSPACE_EMBEDDED_LINK_URL_ALREADY_EXISTS: string;
        WKS_WORKSPACE_EMBEDDED_LINK_NAME_ALREADY_EXISTS: string;
        WKS_APPLICATION_EMBEDDED_LINK_NOT_FOUND: string;
        WKS_APPLICATION_EMBEDDED_LINK_URL_ALREADY_EXISTS: string;
        WKS_APPLICATION_EMBEDDED_LINK_NAME_ALREADY_EXISTS: string;
        WKS_API_WORKSPACE_BAD_REQUEST: string;
        WKS_APPLICATION_CAN_NOT_BE_DELETED: string;
        WKS_CONNECTION_INTERFACE_IN_USE: string;
        WKS_THERE_IS_NO_ENVIRONMENT_IN_ACCOUNT_TO_CONFIGURE_ACTIONS_IN_WORKSPACE: string;
        WKS_THERE_IS_NO_ENVIRONMENT_IN_ACCOUNT_TO_CONFIGURE_PLUGINS_IN_WORKSPACE: string;
        WKS_CONTEXT_STACK_ADD_RELATIONSHIP: string;
        WKS_CONTEXT_STACK_DELETE_RELATIONSHIP: string;
        WKS_APPLICATION_NAME_CONFLICT: string;
        WKS_SHARED_INFRA_NAME_CONFLICT: string;
        WKS_SHARED_INFRA_CAN_NOT_BE_DESTROYED_ERROR: string;
        WKS_ACTIVE_SHARED_INFRA_WITH_DEPLOY_SELF_HOSTED: string;
        WKS_APPLICATION_CAN_NOT_BE_DESTROYED_ERROR: string;
        WKS_ACTIVE_APPLICATION_WITH_DEPLOY_SELF_HOSTED: string;
        WKS_APPLICATION_NOT_FOUND_FOR_THIS_WORKSPACE_ERROR: string;
        WKS_SHARED_INFRA_NOT_FOUND_FOR_THIS_WORKSPACE_ERROR: string;
        WKS_APPLICATION_STATUS_DOES_NOT_ALLOW_RECREATION: string;
        WKS_SHARED_INFRA_STATUS_DOES_NOT_ALLOW_RECREATION: string;
        WKS_DEPLOY_RUNNING: string;
        WKS_WORKFLOW_ACTIONS_UNORDERED_ERROR: string;
        WKS_WORKFLOW_ACTIONS_DUPLICATED_ERROR: string;
        WKS_WORKFLOW_CONTEXT_KEY_DUPLICATED_ERROR: string;
        WKS_WORKFLOW_CONTEXT_NOT_ALLOWED_ERROR: string;
        WKS_WORKFLOW_ACTIONS_CREATE_REPO_DUPLICATED_ERROR: string;
        WKS_WORKFLOW_CONTEXT_MUST_BE_INFORMED_FOR_ALL_ACTIONS_ERROR: string;
        WKS_WORKFLOW_ALREADY_EXISTS_ERROR: string;
        WKS_WORKFLOW_API_MUST_BE_DEFAULT_ERROR: string;
        WKS_WORKFLOW_API_WITH_STACKS_ERROR: string;
        WKS_WORKFLOW_DEFAULT_ALREADY_EXIST_FOR_TYPE_ERROR: string;
        WKS_WORKFLOW_DEFAULT_WITH_STACKS_ERROR: string;
        WKS_WORKFLOW_CONDITION_DUPLICATED_ERROR: string;
        WKS_WORKFLOW_AND_STACK_ARE_NOT_ASSOCIATED_ERROR: string;
        WKS_WORKFLOW_ACTIONS_ARE_NOT_ASSOCIATED_ERROR: string;
        WKS_WORKFLOW_ACTIONS_CONTEXT_NOT_ALLOWED_ERROR: string;
        WKS_WORKFLOW_NOT_FOUND_ERROR: string;
        WKS_STACK_NOT_PRESENT_IN_ACCOUNT: string;
        WKS_STACK_NOT_FOUND_ERROR: string;
        WKS_THERE_IS_ACTIVATED_DEPLOY_FOR_THIS_STACK_VERSION_DELETE_ERROR: string;
        WKS_STACK_VERSION_ID_CONFLICT: string;
        WKS_STK_FILE_NOT_FOUND: string;
        WKS_REPO_URL_NOT_FOUND: string;
        WKS_PLUGINS_TO_UPDATE_NOT_FOUND: string;
        WKS_PLUGINS_NOT_ASSOCIATED_WITH_STACK_VERSION: string;
        WKS_PLUGIN_NOT_ASSOCIATED_WITH_STACK_VERSION: string;
        WKS_PLUGIN_NOT_ASSOCIATED_WITH_SPECIFIC_STACK_VERSION: string;
        WKS_PARAMETER_CONFIG_NOT_FOUND: string;
        WKS_CONTEXT_ATTRIBUTE_NOT_ALLOWED_FOR_ACCOUNT: string;
        WKS_CONTEXT_ATTRIBUTE_SPECIFIC_ERROR: string;
        WKS_CONTEXT_ATTRIBUTE_CONNECTION_TYPE_ERROR: string;
        WKS_CONTEXT_ATTRIBUTE_GENERATED_CONNECTION_TYPE_ERROR: string;
        WKS_CONTEXT_ATTRIBUTE_CANNOT_VARY_PER_ENV: string;
        WKS_STK_FILE_FOUND_BUT_NOT_ASSOCIATED: string;
        SCM_API_4001_GITHUB_GET_REPO_BRANCHES_NOT_FOUND: string;
        SCM_API_4002_GITHUB_GET_REPO_BRANCHES_UNAUTHORIZED: string;
        SCM_API_4002_GITHUB_GET_REPO_BRANCHES_FORBIDDEN: string;
        SCM_API_4021_GITHUB_GET_REPO_TREE_NOT_FOUND: string;
        SCM_API_4022_GITHUB_GET_REPO_TREE_UNAUTHORIZED: string;
        SCM_API_4022_GITHUB_GET_REPO_TREE_FORBIDDEN: string;
        SCM_API_4031_GITHUB_POST_INSTALLATION_NOT_FOUND: string;
        SCM_API_4032_GITHUB_POST_INSTALLATION_UNAUTHORIZED: string;
        SCM_API_4032_GITHUB_POST_INSTALLATION_FORBIDDEN: string;
        SCM_API_4501_GITLAB_GET_PROJECT_NOT_FOUND: string;
        SCM_API_4502_GITLAB_GET_PROJECT_UNAUTHORIZED: string;
        SCM_API_4502_GITLAB_GET_PROJECT_FORBIDDEN: string;
        SCM_API_4511_GITLAB_GET_REPO_BRANCHES_NOT_FOUND: string;
        SCM_API_4512_GITLAB_GET_REPO_BRANCHES_UNAUTHORIZED: string;
        SCM_API_4512_GITLAB_GET_REPO_BRANCHES_FORBIDDEN: string;
        SCM_API_4531_GITLAB_GET_REPO_TREE_NOT_FOUND: string;
        SCM_API_4532_GITLAB_GET_REPO_TREE_UNAUTHORIZED: string;
        SCM_API_4532_GITLAB_GET_REPO_TREE_FORBIDDEN: string;
    };
    pt: {
        WKS_API_WORKSPACE_FORBIDDEN: string;
        WKS_WORKSPACE_NOT_FOUND: string;
        WKS_CONNECTION_INTERFACE_ALREADY_EXISTS_FOR_ACCOUNT: string;
        WKS_CONNECTION_INTERFACE_NOT_AVAILABLE: string;
        WKS_WORKSPACE_ALREADY_EXISTS_FOR_ACCOUNT: string;
        WKS_THERE_IS_A_SHARED_INFRA_WITH_THIS_STACK_DELETE_ERROR: string;
        WKS_THERE_IS_AN_APPLICATION_WITH_THIS_STACK_DELETE_ERROR: string;
        WKS_APPLICATION_LINK_NAME_ALREADY_EXISTS: string;
        WKS_APPLICATION_LINK_URL_ALREADY_EXISTS: string;
        WKS_SHARED_INFRA_LINK_NAME_ALREADY_EXISTS: string;
        WKS_SHARED_INFRA_LINK_URL_ALREADY_EXISTS: string;
        WKS_WORKSPACE_LINK_NAME_ALREADY_EXISTS: string;
        WKS_WORKSPACE_LINK_URL_ALREADY_EXISTS: string;
        WKS_ATTR_INVALID: string;
        WKS_ATTR_INVALID_EVALUATED_FROM_VALIDATIONS_LENGTH: string;
        WKS_BAD_REQUEST_ERROR: string;
        WKS_THERE_ARE_CLOUD_ACCOUNT_FOR_THIS_WORKSPACE_DELETE_ERROR: string;
        WKS_THERE_ARE_STACK_IN_THIS_WORKSPACE_DELETE_ERROR: string;
        WKS_THERE_ARE_CONNECTION_INTERFACE_IN_THIS_WORKSPACE_DELETE_ERROR: string;
        WKS_THERE_IS_RUNNING_DEPLOY_FOR_THIS_APPLICATION: string;
        WKS_THERE_ARE_ACTIVATED_DEPLOY_FOR_THIS_APPLICATION: string;
        WKS_THERE_IS_CONNECTION_INTERFACE_FOR_THIS_APPLICATION_DELETE_ERROR: string;
        WKS_THERE_IS_RUNNING_DEPLOY_FOR_THIS_SHARED_INFRA: string;
        WKS_THERE_ARE_ACTIVATED_DEPLOY_FOR_THIS_SHARED_INFRA: string;
        WKS_THERE_IS_CONNECTION_INTERFACE_FOR_THIS_SHARED_INFRA_DELETE_ERROR: string;
        WKS_WORKSPACE_EMBEDDED_LINK_NOT_FOUND: string;
        WKS_WORKSPACE_EMBEDDED_LINK_URL_ALREADY_EXISTS: string;
        WKS_WORKSPACE_EMBEDDED_LINK_NAME_ALREADY_EXISTS: string;
        WKS_APPLICATION_EMBEDDED_LINK_NOT_FOUND: string;
        WKS_APPLICATION_EMBEDDED_LINK_URL_ALREADY_EXISTS: string;
        WKS_APPLICATION_EMBEDDED_LINK_NAME_ALREADY_EXISTS: string;
        WKS_API_WORKSPACE_BAD_REQUEST: string;
        WKS_APPLICATION_CAN_NOT_BE_DELETED: string;
        WKS_CONNECTION_INTERFACE_IN_USE: string;
        WKS_THERE_IS_NO_ENVIRONMENT_IN_ACCOUNT_TO_CONFIGURE_ACTIONS_IN_WORKSPACE: string;
        WKS_THERE_IS_NO_ENVIRONMENT_IN_ACCOUNT_TO_CONFIGURE_PLUGINS_IN_WORKSPACE: string;
        WKS_CONTEXT_STACK_ADD_RELATIONSHIP: string;
        WKS_CONTEXT_STACK_DELETE_RELATIONSHIP: string;
        WKS_APPLICATION_NAME_CONFLICT: string;
        WKS_SHARED_INFRA_NAME_CONFLICT: string;
        WKS_SHARED_INFRA_CAN_NOT_BE_DESTROYED_ERROR: string;
        WKS_ACTIVE_SHARED_INFRA_WITH_DEPLOY_SELF_HOSTED: string;
        WKS_APPLICATION_CAN_NOT_BE_DESTROYED_ERROR: string;
        WKS_ACTIVE_APPLICATION_WITH_DEPLOY_SELF_HOSTED: string;
        WKS_APPLICATION_NOT_FOUND_FOR_THIS_WORKSPACE_ERROR: string;
        WKS_SHARED_INFRA_NOT_FOUND_FOR_THIS_WORKSPACE_ERROR: string;
        WKS_APPLICATION_STATUS_DOES_NOT_ALLOW_RECREATION: string;
        WKS_SHARED_INFRA_STATUS_DOES_NOT_ALLOW_RECREATION: string;
        WKS_DEPLOY_RUNNING: string;
        WKS_WORKFLOW_ACTIONS_UNORDERED_ERROR: string;
        WKS_WORKFLOW_ACTIONS_DUPLICATED_ERROR: string;
        WKS_WORKFLOW_CONTEXT_KEY_DUPLICATED_ERROR: string;
        WKS_WORKFLOW_CONTEXT_NOT_ALLOWED_ERROR: string;
        WKS_WORKFLOW_ACTIONS_CREATE_REPO_DUPLICATED_ERROR: string;
        WKS_WORKFLOW_CONTEXT_MUST_BE_INFORMED_FOR_ALL_ACTIONS_ERROR: string;
        WKS_WORKFLOW_ALREADY_EXISTS_ERROR: string;
        WKS_WORKFLOW_API_MUST_BE_DEFAULT_ERROR: string;
        WKS_WORKFLOW_API_WITH_STACKS_ERROR: string;
        WKS_WORKFLOW_DEFAULT_ALREADY_EXIST_FOR_TYPE_ERROR: string;
        WKS_WORKFLOW_DEFAULT_WITH_STACKS_ERROR: string;
        WKS_WORKFLOW_CONDITION_DUPLICATED_ERROR: string;
        WKS_WORKFLOW_AND_STACK_ARE_NOT_ASSOCIATED_ERROR: string;
        WKS_WORKFLOW_ACTIONS_ARE_NOT_ASSOCIATED_ERROR: string;
        WKS_WORKFLOW_ACTIONS_CONTEXT_NOT_ALLOWED_ERROR: string;
        WKS_WORKFLOW_NOT_FOUND_ERROR: string;
        WKS_STACK_NOT_PRESENT_IN_ACCOUNT: string;
        WKS_STACK_NOT_FOUND_ERROR: string;
        WKS_THERE_IS_ACTIVATED_DEPLOY_FOR_THIS_STACK_VERSION_DELETE_ERROR: string;
        WKS_STACK_VERSION_ID_CONFLICT: string;
        WKS_STK_FILE_NOT_FOUND: string;
        WKS_REPO_URL_NOT_FOUND: string;
        WKS_PLUGINS_TO_UPDATE_NOT_FOUND: string;
        WKS_PLUGINS_NOT_ASSOCIATED_WITH_STACK_VERSION: string;
        WKS_PLUGIN_NOT_ASSOCIATED_WITH_STACK_VERSION: string;
        WKS_PLUGIN_NOT_ASSOCIATED_WITH_SPECIFIC_STACK_VERSION: string;
        WKS_PARAMETER_CONFIG_NOT_FOUND: string;
        WKS_CONTEXT_ATTRIBUTE_NOT_ALLOWED_FOR_ACCOUNT: string;
        WKS_CONTEXT_ATTRIBUTE_SPECIFIC_ERROR: string;
        WKS_CONTEXT_ATTRIBUTE_CONNECTION_TYPE_ERROR: string;
        WKS_CONTEXT_ATTRIBUTE_GENERATED_CONNECTION_TYPE_ERROR: string;
        WKS_CONTEXT_ATTRIBUTE_CANNOT_VARY_PER_ENV: string;
        WKS_STK_FILE_FOUND_BUT_NOT_ASSOCIATED: string;
        SCM_API_4001_GITHUB_GET_REPO_BRANCHES_NOT_FOUND: string;
        SCM_API_4002_GITHUB_GET_REPO_BRANCHES_UNAUTHORIZED: string;
        SCM_API_4002_GITHUB_GET_REPO_BRANCHES_FORBIDDEN: string;
        SCM_API_4021_GITHUB_GET_REPO_TREE_NOT_FOUND: string;
        SCM_API_4022_GITHUB_GET_REPO_TREE_UNAUTHORIZED: string;
        SCM_API_4022_GITHUB_GET_REPO_TREE_FORBIDDEN: string;
        SCM_API_4031_GITHUB_POST_INSTALLATION_NOT_FOUND: string;
        SCM_API_4032_GITHUB_POST_INSTALLATION_UNAUTHORIZED: string;
        SCM_API_4032_GITHUB_POST_INSTALLATION_FORBIDDEN: string;
        SCM_API_4501_GITLAB_GET_PROJECT_NOT_FOUND: string;
        SCM_API_4502_GITLAB_GET_PROJECT_UNAUTHORIZED: string;
        SCM_API_4502_GITLAB_GET_PROJECT_FORBIDDEN: string;
        SCM_API_4511_GITLAB_GET_REPO_BRANCHES_NOT_FOUND: string;
        SCM_API_4512_GITLAB_GET_REPO_BRANCHES_UNAUTHORIZED: string;
        SCM_API_4512_GITLAB_GET_REPO_BRANCHES_FORBIDDEN: string;
        SCM_API_4531_GITLAB_GET_REPO_TREE_NOT_FOUND: string;
        SCM_API_4532_GITLAB_GET_REPO_TREE_UNAUTHORIZED: string;
        SCM_API_4532_GITLAB_GET_REPO_TREE_FORBIDDEN: string;
    };
};
//# sourceMappingURL=workspace.d.ts.map