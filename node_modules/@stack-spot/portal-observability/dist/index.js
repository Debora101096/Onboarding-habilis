...
import { getCommonProps, prepareEvent, prepareRouteDetails } from './event-bus-events.js';
const apis = {
    'stackspot': apisJson,
    'itau': apisItauJson,
};
 ...
 */
export async function datadogInit({ applicationId, clientToken, env, service, tenant = 'stackspot', ...rest }) {
    const allowedAddresses = Object.values(apis[tenant]).map(it => it.url?.[env] ?? it.url.prd);
    const allowedTracingUrls = (value) => allowedAddresses.some(it => value.startsWith(it));
    if (clientToken) {
        datadogRum.init({
            applicationId,
            clientToken,
 ...
 */
export async function trackAction(action, context) {
    datadogRum.addAction(action, context);
}
let targetOfLastTrackedEvent;
/**
...
export function trackEvent(event, context) {
    ...
    trackAction(targetOfLastTrackedEvent.id || targetOfLastTrackedEvent.text || targetOfLastTrackedEvent.icon || targetOfLastTrackedEvent.tag || 'unknown', targetOfLastTrackedEvent);
}
/**
...
export async function sendClickToEventBus(itemText, routeDetails, portal, sendToEventBus = true) {
...
async function sendPageLoadedToEventBus(routeDetails, portal, customerId, sendToEventBus = true) {
...
export async function trackPageLoaded(routeDetails, portal, customerId, sendToEventBus) {
...
export function formatRouteForAnalytics(path, params) {
 ...
 */
export function getRouteForAnalytics({ route, params }) {
 ...
 */
export function usePageTracking(path, params, portal) {
    const session = useSession();
    useEffect(() => {
        const sub = session?.getTokenData().sub;
        if (!sub)
            return;
