...
import { omitBy, random as random$1, deburr, map, snakeCase, sortBy, uniqBy, cloneDeep } from 'lodash';
...
class InterceptorAuth {
  ...
  async interceptRequest(request) {
    ...
    const headers = omitBy(this.normalizeHeaders((_a = request.init) == null ? void 0 : _a.headers), (_, key) => key.toLowerCase() === "authorization");
...
function randomizeString(numberOfChars) {
  const rand = random$1(0, numberOfSymbols - 1);
...
class ThirdPartyClient {
  ...
  createSlug(name) {
    return deburr(name).toLowerCase().replaceAll(/^-+|-+$/g, "").replace(/ /g, "-").replace(/(-)+/g, "-").replace(/[^a-z-\d]/g, "").substring(0, 60);
  }
...
export { AuthManager, AuthMethodUnavailable, AuthenticationError, AuthenticationTimeout, ConnectionError, InterceptorAuth, InvalidCredentialsError, InvalidProvider, InvalidSSOError, InvalidTenantError, LinkedList, LogoutError, MalFormedAccessToken, NetworkError, PendingAuthenticationError, Session, SessionExpiredError, ThirdPartyClient, ThirdPartyObservable, TokenClient };
