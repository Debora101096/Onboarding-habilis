...
export class ThirdPartyClient {
  ...
  async findAuthDataByEmail(email: string): Promise<ThirdPartyAuthData> {
  ...
  private buildSearchParams({ codeChallenge, state, email, idp, locale, type }: ThirdPartyAuthRequest) {
    ...
    if (type === 'idp' && !!idp) { params.login_hint = idp }
  ...
  */
  ...
  async completeThirdPartyLogin(queryString: string): Promise<{ sessionData: SessionPayload, data?: unknown }> {
    ...
    const { codeVerifier, tenant, data, provider }: Partial<StorageData> = fromStorage ? JSON.parse(fromStorage) : {}
    ...
    return { sessionData: { ...finalSessionData, provider }, data }
  }
