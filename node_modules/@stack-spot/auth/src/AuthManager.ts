...
export class AuthManager<Data = undefined> {
  ...
  constructor(config: AuthConfig) {
    ...
    this.config = {
      ...
      logger: config.logger ?? new Logger(storage)
    }
    try {
      // @ts-ignore we want to make the logger available for now, so we can easily ask the user to provide us logs.
      window.authenticationLogger = this.config.logger
    } catch { /* empty */ }
    if (this.config.logger instanceof Logger) this.config.logger.load()
   ...
   */
  startDeviceLogin(loginData: DeviceLoginData): Omit<ThirdPartyObservable<undefined>, 'onRedirect' | 'trigger'> {
    const observable = new ThirdPartyObservable<undefined>()
    const start = async () => {
      try {
        const { tenant } = (!loginData.tenant && loginData.email)
          ? await this.thirdPartyClient.findAuthDataByEmail(loginData.email)
          : { tenant: loginData.tenant || this.thirdPartyClient.config.defaultTenant }
        const authData = await this.tokenClient.getDeviceAuthUrl({ ...loginData, tenant })
        observable.trigger('url', authData.verification_uri_complete)
        const expiration = new Date().getTime() + authData.expires_in * 1000
        const checkAuth = () => {
          setTimeout(async () => {
            try {
              ...
              observable.trigger('success', new Session(sessionData, this.config, this.tokenClient), undefined)
            } catch (error) {
              if (new Date().getTime() > expiration) {
                const error = new AuthenticationTimeout()
