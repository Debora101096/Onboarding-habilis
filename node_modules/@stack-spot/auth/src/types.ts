...
export type GenericStorage = Pick<Storage, 'clear' | 'getItem' | 'key' | 'removeItem' | 'setItem' | 'length'>
...
export interface SessionPayload {
  ...
  /**
   * Time in milliseconds when this payload was generated.
   */
...
export interface AccessTokenPayload extends JWTPayload {
  ...
  readonly trial_account_status?: 'PENDING' | 'REGISTERED' | 'EXPIRED',
  readonly client_id?: string,
...
interface DeviceCredentials extends Credentials {
  device_code: string,
}

export type CredentialMethod = 'password' | 'client' | 'refreshToken' | 'authorizationCode' | 'device'
...
export interface ThirdPartyAuthData {
  tenant: string,
  idp?: ThirdPartyProvider,
  accountType: string,
  iamTenant?: string,
}

export type ThirdPartyAuthType = 'sso' | 'idp'
export type ThirdPartyProvider = `external-idp:${ExternalLoginPartner}`
...
export interface DeviceAuth {
  /**
   * Identifies this login attempt.
   */
  device_code: string,
  userCode: string,
  verificationUri: string,
  /**
   * Url for the user to login.
   */
  verification_uri_complete: string,
  /**
   * Time (in seconds) the login attempt should still be considered valid.
   */
  expires_in: number,
  /**
   * Interval (in seconds) in which the calls to check if the login is successful should be sent.
   */
  interval: number,
}
...
export interface DeviceLoginData {
  email?: string,
  clientId?: string,
  provider?: ExternalLoginPartner,
  tenant?: string,
}

export type ExternalLoginPartner = 'google' | 'microsoft' | 'github' | 'email'
